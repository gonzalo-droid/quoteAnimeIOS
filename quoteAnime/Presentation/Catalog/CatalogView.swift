...
#Preview("CatalogView") {
    let repo = PreviewQuoteRepository()
    return CatalogView(
        getAllQuotesUseCase: GetAllQuotesUseCase(repository: repo),
        getFavoriteQuotesUseCase: GetFavoriteQuotesUseCase(repository: repo),
...
#Preview("CircleActionButton") {
    CircleActionButton(icon: "heart.fill", color: .red) {}
        .padding()
        .background(Color.bgDark)
}
