...
| **Feed de Inicio filtrado por animes** | El feed mostraba todas las frases: la selección sólo afectaba a las notificaciones y al widget (y además no había forma de cambiarla) | `GetAllQuotesUseCase.execute(filteredBy:)` filtra el feed con la selección, y `HomeViewModel.reloadIfCategorySelectionChanged()` lo recarga al volver de Ajustes | Decisión del usuario en iOS: "si elegí tres animes, quiero ver esos tres". **Resuelto: Android se alineó en `02bf6e0` (2026-09-18) — `HomeViewModel` observa `selectedCategoryIds` y filtra con `Quote.isInCategories` (la misma regla que la notificación y el widget), el pager vuelve arriba al cambiar la selección; ya no es divergencia.** Diferencias que siguen: iOS baraja el feed y Android lo muestra en el orden del RTDB; y en producción `isInCategories` compara con las **emociones** de `categories`, así que el mismo mecanismo filtra por emoción en Android y por anime en iOS (ver `selectedCategoryIds`). |
| **`selectedCategoryIds`** | Valores del campo `categories` de cada frase en la **Realtime Database** (`QuoteRemoteDataSource.getCategories()`: usa `categories` si la frase lo tiene y sólo si no, su `anime`). Las 307 frases de producción traen `categories` (consultado por REST el 2026-09-18), así que en la práctica Android guarda **emociones** — 10: `reflexión`, `motivación`, `lucha`, `esperanza`, `orgullo`, `tristeza`, `sacrificio`, `amistad`, `amor`, `soledad` — aunque la sección de Ajustes se llame "Animes" | Nombres de anime (campo `anime`, 19 en producción) | Siguen siendo espacios de valores distintos (emociones frente a animes): copiar la selección de un lado al otro la vaciaría en silencio, porque ningún valor coincide. Y aunque algún día coincidieran, el valor tampoco cruza porque cada instalación guarda el suyo (DataStore en Android, `UserDefaults` + App Group en iOS) y ningún backend lo sincroniza. **Corregido en el bloque de mejoras de iOS (2026-09-18)**: la fila decía "ids de Firestore"; la app no usa Firestore. Ver deuda de Android: el selector "Animes" de Android muestra emociones. |
...
- `selectedCategoryIds` — Android guarda valores del campo `categories` de la Realtime Database
  (emociones en producción: `motivación`, `amor`…); iOS guarda nombres de anime. Ningún valor
  coincide, así que copiar la selección la vaciaría en silencio. Además es una preferencia local de
  cada instalación, sin backend que la sincronice. (Hasta el 2026-09-18 aquí faltaba y la tabla de
  divergencias decía "ids de Firestore": la app no usa Firestore.)
