...
| **Feed de Inicio filtrado por animes** | El feed mostraba todas las frases: la selección sólo afectaba a las notificaciones y al widget (y además no había forma de cambiarla) | `GetAllQuotesUseCase.execute(filteredBy:)` filtra el feed con la selección, y `HomeViewModel.reloadIfCategorySelectionChanged()` lo recarga al volver de Ajustes | Decisión del usuario en iOS: "si elegí tres animes, quiero ver esos tres". **Resuelto: Android se alineó en `02bf6e0` (2026-09-18) — `HomeViewModel` observa `selectedCategoryIds` y filtra con `Quote.isInCategories` (hoy `Quote.isFromAnimes`; la misma regla que la notificación y el widget), el pager vuelve arriba al cambiar la selección; ya no es divergencia.** Diferencia que sigue: iOS baraja el feed y Android lo muestra en el orden del RTDB. (Hasta `6e3d39a` Android comparaba con las emociones de `categories`; desde entonces filtra por `anime` con `Quote.isFromAnimes`, igual que iOS.) |
| **`selectedCategoryIds`** | Nombres de anime (campo `anime` de la **Realtime Database**) desde `6e3d39a` (2026-09-18). Antes guardaba valores de `categories` — **emociones** en producción (`motivación`, `amor`…) — aunque la sección se llamara "Animes"; esos valores viejos se descartan al abrir la app (`ReconcileAnimeSelectionUseCase`; si no queda ninguno, "todos") | Nombres de anime (campo `anime`, 19 en producción) | **Resuelto en `6e3d39a`: ambos guardan nombres de anime**, ya no son espacios de valores distintos. El valor igual no cruza de plataforma: es una preferencia local de cada instalación (DataStore en Android, `UserDefaults` + App Group en iOS) y ningún backend la sincroniza. Diferencia menor: Android descarta un `anime` vacío de la lista; iOS lo mostraría. **Corregido en el bloque de mejoras de iOS (2026-09-18)**: la fila decía "ids de Firestore"; la app no usa Firestore. |
...
| **Tamaño de letra del sistema (Dynamic Type)** | Todo el texto en `sp`: escala con la preferencia del sistema sin tope, en todas las pantallas y widgets | Desde el bloque de mejoras de iOS (2026-09-18): el texto escala (estilos de texto o `scaledFont`, ver `Theme/Typography.swift`), con topes locales donde el marco es fijo — frase a pantalla completa, calendario y vista previa de sugerencia en `xxxLarge`, barras superiores propias en `xxxLarge`, mapa de actividad en `xxLarge`, widgets de Mi Rutina en `xLarge` y **widgets de frases sin escalar**. Los SF Symbols sueltos (chevrons, corazón, íconos en círculos) conservan su tamaño | Los topes siguen lo que hace el sistema con sus barras y evitan texto cortado en marcos fijos; los widgets de frases colapsaban la frase a una línea en `xLarge` (visto en el simulador). En el tamaño por defecto la app es idéntica píxel a píxel a la anterior (comparado en 8 pantallas). |
...
- `selectedCategoryIds` — desde `6e3d39a` (2026-09-18) ambas plataformas guardan nombres de anime
  (campo `anime`); antes Android guardaba emociones de `categories`. Aun así no cruza: es una
  preferencia local de cada instalación (DataStore en Android, `UserDefaults` + App Group en iOS),
  sin backend que la sincronice. (Hasta el 2026-09-18 aquí faltaba y la tabla de divergencias decía
  "ids de Firestore": la app no usa Firestore.)
...
| `SettingsScreen.AnimeSelectionSection` + `QuoteRemoteDataSource.getCategories()` | La sección se titula "Animes" ("Elige de qué animes…", "Todos los animes"), pero los chips salen de `getCategories()`, que prefiere el campo `categories` de cada frase y sólo cae a `anime` si falta. En producción las 307 frases lo tienen, así que el usuario ve **10 emociones** (`amistad`, `amor`, `esperanza`…) bajo "Animes", y el feed, la notificación y el widget filtran por emoción. Encontrado por lectura y con los datos de producción (REST, 2026-09-18); **no reproducido** en un emulador. Decisión del usuario: el selector es de **animes**, como iOS. **Arreglado en Android (`6e3d39a`, 2026-09-18).** `getCategories()` ya no existe: la lista sale del campo `anime` (`GetAnimesUseCase`, orden alfabético como iOS) y el feed, la notificación y el widget comparan con `anime` (`Quote.isFromAnimes`); las emociones quedan sólo para el filtro del Catálogo. Una selección guardada con emociones se limpia al abrir la app: si no queda ningún anime vuelve a "todos" (`ReconcileAnimeSelectionUseCase`). |
